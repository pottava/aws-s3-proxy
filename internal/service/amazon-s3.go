...
//S3ParallelGet(f io.Writer,bucket,key string, rangeHeader *string)error
...
func (d *downloader) download() (n int64, err error) {
	...
	if total := d.getTotalBytes(); total >= 0 {
		...
		for d.getErr() == nil {
			if d.pos >= total {
				fmt.Println("send data failed: ", d.getErr())
...
func (d *downloader) tryDownloadChunk(in *s3.GetObjectInput, w *dlchunk) (int64, error) {
	...
	fmt.Println("S3 object info: ", *resp)
	if err != nil {
		fmt.Println("get s3 object err: ", err)
		return 0, err
	}
	...
	for d.index != w.num {
		// fmt.Println("wait before data write. ", *d.index)
		...
		if d.index == w.num {
			...
			num := atomic.AddInt64(&d.index, 1)
			fmt.Println("index new value: ", num)
			break
		}
	}
...
func (d *downloader) downloadPart(ch chan dlchunk) {
	...
	for {
		...
		fmt.Println("start channel download: ", chunk.num)
